...
export function LedgerTable() {
  ...
  return (
    <div className="space-y-3">
      <div className="overflow-hidden rounded-xl border border-border bg-card dark:border-border/60 dark:bg-card/30">
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.id} className="border-t border-border dark:border-border/30">
                ...
                <td className={`px-4 py-2.5 text-right font-mono ${r.delta >= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-rose-600 dark:text-rose-400"}`}>
      ...
      {hasNextPage && (
        <button
          ...
          className="w-full rounded-lg border border-border bg-card px-4 py-2 text-sm text-muted-foreground hover:bg-accent dark:border-border/60 dark:bg-card/30 dark:hover:bg-card/60"
        >
